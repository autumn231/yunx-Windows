...
/**
 * 下载页 ViewModel（桌面版）：任务列表（Room Flow → StateFlow）+ 实时统计 + 操作转发。
 * 自上游 Android 版移植（去掉 androidx.lifecycle，进程常驻单例）。
 */
class DownloadViewModel(private val manager: DownloadManager) {
    ...
    fun enqueue(url: String, fileName: String) {
        scope.launch {
            runCatching { manager.enqueue(url = url, fileName = fileName) }
                .onSuccess { SnackbarController.show("已添加下载任务") }
                .onFailure { SnackbarController.show("添加失败：${it.message}") }
        }
    }
